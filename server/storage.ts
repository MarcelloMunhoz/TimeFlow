...
export class DatabaseStorage implements IStorage {
  ...
  async updateProjectSubphase(id: number, updates: any): Promise<any | undefined> {
    try {
      console.log(`🔄 Updating project subphase ${id} with data:`, updates);
      
      // Get current subphase data before update to check for date changes
      const currentSubphase = await this.getProjectSubphase(id);
      if (!currentSubphase) {
        console.log(`❌ Project subphase ${id} not found`);
        return undefined;
      }

      const [updatedSubphase] = await db.update(projectSubphases)
        .set(updates)
        .where(eq(projectSubphases.id, id))
        .returning();

      if (!updatedSubphase) {
        console.log(`❌ Failed to update project subphase ${id}`);
        return undefined;
      }

      console.log(`✅ Project subphase ${id} updated successfully`);

      // If endDate was set and it wasn't set before, create automatic appointment
      if (updates.endDate && !currentSubphase.endDate) {
        console.log(`📅 End date set for subphase ${id}, creating automatic appointment...`);
        await this.createAutomaticSubphaseAppointment(updatedSubphase);
      }
  ...
  // Create automatic appointment when subphase end date is set
  async createAutomaticSubphaseAppointment(subphase: any): Promise<void> {
    try {
      console.log(`🤖 Creating automatic appointment for subphase ${subphase.id}`);

      // Get subphase details with related data
      const subphaseDetails = await db
        .select({
          subphase: subphases,
          projectPhase: projectPhases,
          project: projects,
        ...
        .from(projectSubphases)
        .innerJoin(subphases, eq(projectSubphases.subphaseId, subphases.id))
        .innerJoin(projectPhases, eq(projectSubphases.projectPhaseId, projectPhases.id))
        .innerJoin(projects, eq(projectPhases.projectId, projects.id))
        .innerJoin(phases, eq(projectPhases.phaseId, phases.id))
        .where(eq(projectSubphases.id, subphase.id))
        .limit(1);

      if (subphaseDetails.length === 0) {
        console.log(`❌ Could not find subphase details for ID ${subphase.id}`);
        return;
      }

      const details = subphaseDetails[0];
      const appointmentDate = subphase.endDate;

      // Find available time slot within work hours (8h-18h, excluding 12h-13h)
      const availableTime = await this.findAvailableTimeSlot(appointmentDate);

      // Create appointment data
      const appointmentData = {
        title: `Conclusão: ${details.subphase.name}`,
        description: `Agendamento automático para conclusão da subfase "${details.subphase.name}" do projeto "${details.project.name}" - Fase: ${details.phase.name}`,
        date: appointmentDate,
        startTime: availableTime,
        durationMinutes: 60, // 1 hour as requested
        slaMinutes: 120, // 2 hours SLA as requested
        projectId: details.project.id,
        projectSubphaseId: subphase.id,
        phaseId: details.phase.id,
        assignedUserId: subphase.assignedUserId,
        priority: 'high',
        category: 'subfase_conclusao',
        notes: `Agendamento criado automaticamente para conclusão da subfase. Data limite: ${appointmentDate}`,
        allowOverlap: true, // Allow as "encaixe" if there are conflicts
      ...
      console.log(`✅ Automatic appointment created with ID: ${appointment.id}`);

    } catch (error) {
      console.error(`❌ Error creating automatic appointment for subphase ${subphase.id}:`, error);
      // Don't throw error to avoid breaking subphase update if appointment creation fails
      console.log(`⚠️ Continuing despite appointment creation failure`);
    }
  }

  // Find available time slot within work hours
  async findAvailableTimeSlot(date: string): Promise<string> {
    try {
      console.log(`🔍 Finding available time slot for date: ${date}`);

      // Define work hour slots (avoiding lunch 12:00-13:00)
      const workSlots = [
        { start: '08:00', end: '12:00' },
        { start: '13:00', end: '18:00' }
      ];

      // Check each work slot for availability
      for (const slot of workSlots) {
        const slotStart = this.timeToMinutes(slot.start);
        const slotEnd = this.timeToMinutes(slot.end);
        
        // Try to find a 1-hour slot within this work period
        for (let time = slotStart; time <= slotEnd - 60; time += 30) { // Check every 30 minutes
          const timeStr = this.minutesToTime(time);
          const endTimeStr = this.minutesToTime(time + 60);
          
          // Check if this time slot has conflicts
          const hasConflict = await this.checkTimeConflict(date, timeStr, endTimeStr);
          
          if (!hasConflict) {
            console.log(`✅ Found available slot: ${timeStr} - ${endTimeStr}`);
            return timeStr;
          }
        }
      }

      // If no slot available, default to 08:00 as "encaixe"
      console.log(`⚠️ No available slots found, defaulting to 08:00 as encaixe`);
      return '08:00';
  ...
  async updateProjectSubphaseLegacy(id: number, updates: any): Promise<any | undefined> {
    try {
      const fields = [];
      const values = [];
      ...
      if (updatedSubphase) {
        await this.updatePhaseProgress(updatedSubphase.project_phase_id);
      }

      return updatedSubphase;
    } catch (error) {
      console.error("Error in updateProjectSubphaseLegacy:", error);
      throw error;
    }
  }
  ...
  // Get all project subphases for a project (across all phases)
  async getProjectSubphasesByProject(projectId: number): Promise<any[]> {
    try {
      console.log(`🔍 Getting all subphases for project ${projectId}`);
      
      const result = await db
        .select({
          id: projectSubphases.id,
          projectPhaseId: projectSubphases.projectPhaseId,
          subphaseId: projectSubphases.subphaseId,
          startDate: projectSubphases.startDate,
          endDate: projectSubphases.endDate,
          actualStartDate: projectSubphases.actualStartDate,
          actualEndDate: projectSubphases.actualEndDate,
          status: projectSubphases.status,
          progressPercentage: projectSubphases.progressPercentage,
          assignedUserId: projectSubphases.assignedUserId,
          priority: projectSubphases.priority,
          estimatedHours: projectSubphases.estimatedHours,
          actualHours: projectSubphases.actualHours,
          qualityScore: projectSubphases.qualityScore,
          notes: projectSubphases.notes,
          createdAt: projectSubphases.createdAt,
          subphase: {
            id: subphases.id,
            name: subphases.name,
            description: subphases.description,
            estimatedDurationDays: subphases.estimatedDurationDays,
            orderIndex: subphases.orderIndex
          }
        })
        .from(projectSubphases)
        .innerJoin(subphases, eq(projectSubphases.subphaseId, subphases.id))
      ...
      console.log(`✅ Found ${result.length} subphases for project ${projectId}`);
      return result;
    } catch (error) {
      console.error("Error in getProjectSubphasesByProject:", error);
      throw error;
    }
  }
  ...
  // ===== DAILY SCHEDULE EXPORT METHODS =====

  // Get daily schedule with detailed information for export
  async getDailyScheduleForExport(date: string): Promise<any> {
    try {
      console.log(`📅 Getting daily schedule for export: ${date}`);
        ...
        appointmentsList.map(async (apt) => {
          ...
          if (apt.projectId) {
            const [project] = await db
              ...
              .from(projects)
              .where(eq(projects.id, apt.projectId))
              .limit(1);
          ...
          if (apt.companyId) {
            const [company] = await db
              ...
              .from(companies)
              .where(eq(companies.id, apt.companyId))
              .limit(1);
          ...
          if (apt.assignedUserId) {
            const [user] = await db
              ...
              .from(users)
              .where(eq(users.id, apt.assignedUserId))
              .limit(1);
            enriched.assignedUser = user;
          }
          ...
          if (apt.phaseId) {
            const [phase] = await db
              ...
              .from(phases)
              .where(eq(phases.id, apt.phaseId))
              .limit(1);
      ...
      const scheduledAppointments = enrichedAppointments.filter(apt => apt.status === 'scheduled').length;
      const completedAppointments = enrichedAppointments.filter(apt => apt.status === 'completed').length;
      const pomodoroSessions = enrichedAppointments.filter(apt => apt.isPomodoro).length;
      const overtimeAppointments = enrichedAppointments.filter(apt => apt.isOvertime).length;
      const totalDurationMinutes = enrichedAppointments.reduce((sum, apt) => sum + apt.durationMinutes, 0);

      // Get unique projects and companies
      const uniqueProjects = [...new Set(enrichedAppointments.filter(apt => apt.project).map(apt => apt.project.name))];
      const uniqueCompanies = [...new Set(enrichedAppointments.filter(apt => apt.company).map(apt => apt.company.name))];
      ...
      console.error("Error getting daily schedule for export:", error);
      throw error;
    }
  }

  // Export daily schedule as formatted text with emojis
  async exportDailySchedule(date: string, format: string = 'text'): Promise<string> {
    try {
      console.log(`📄 Exporting daily schedule for ${date} in ${format} format`);

      const scheduleData = await this.getDailyScheduleForExport(date);
      const { appointments, summary } = scheduleData;
      ...
      // Get emoji for appointment category/type
      ...
      // Header
      exportText += `📅 CRONOGRAMA DIÁRIO - ${formatDate(date).toUpperCase()}\n`;
      exportText += `═══════════════════════════════════════════════════════\n\n`;

      // Summary
      exportText += `📊 RESUMO DO DIA:\n`;
      exportText += `• 📝 Total de agendamentos: ${summary.totalAppointments}\n`;
      exportText += `• ⏰ Agendados: ${summary.scheduledAppointments}\n`;
      exportText += `• ✅ Concluídos: ${summary.completedAppointments}\n`;
      exportText += `• 🍅 Pomodoros: ${summary.pomodoroSessions}\n`;
      exportText += `• ⏳ Tempo total: ${summary.totalDurationHours}h (${summary.totalDurationMinutes}min)\n`;
      
      if (summary.overtimeAppointments > 0) {
        exportText += `• ⚠️ Encaixe/Overtime: ${summary.overtimeAppointments}\n`;
      }

      if (summary.uniqueProjects.length > 0) {
        exportText += `• 🎯 Projetos: ${summary.uniqueProjects.join(', ')}\n`;
      }

      if (summary.uniqueCompanies.length > 0) {
        exportText += `• 🏢 Empresas: ${summary.uniqueCompanies.join(', ')}\n`;
      }
      ...
      if (appointments.length === 0) {
        exportText += `🎉 AGENDA LIVRE!\n`;
        exportText += `Não há agendamentos para este dia.\n`;
      } else {
        exportText += `📋 AGENDA DETALHADA:\n`;
        exportText += `───────────────────────────────────────────────────────\n\n`;

        appointments.forEach((apt: any, index: number) => {
          const emoji = getAppointmentEmoji(apt);
          const statusEmoji = getStatusEmoji(apt.status);
          const priorityEmoji = getPriorityEmoji(apt.priority);

          exportText += `${emoji} ${apt.startTime} - ${apt.endTime} (${apt.durationMinutes}min)\n`;
          exportText += `   📌 ${apt.title}\n`;

          if (apt.description) {
            exportText += `   💭 ${apt.description}\n`;
          }

          // Status and priority
          exportText += `   ${statusEmoji} Status: ${apt.status}`;
          if (apt.priority && apt.priority !== 'medium') {
            exportText += ` | ${priorityEmoji} Prioridade: ${apt.priority}`;
          }
          exportText += `\n`;

          // Project and company info
          if (apt.project) {
            exportText += `   🎯 Projeto: ${apt.project.name}\n`;
          }
          if (apt.company) {
            exportText += `   🏢 Empresa: ${apt.company.name}\n`;
          }

          // Phase info
          if (apt.phase) {
            exportText += `   📊 Fase: ${apt.phase.name}\n`;
          }

          // Assigned user
          if (apt.assignedUser) {
            exportText += `   👤 Responsável: ${apt.assignedUser.name}\n`;
          }

          // Location or meeting URL
          if (apt.location) {
            exportText += `   📍 Local: ${apt.location}\n`;
          }
          if (apt.meetingUrl) {
            exportText += `   💻 Link: ${apt.meetingUrl}\n`;
          }

          // SLA info
          if (apt.slaMinutes) {
            const slaHours = Math.round((apt.slaMinutes / 60) * 10) / 10;
            exportText += `   ⏱️ SLA: ${slaHours}h (${apt.slaMinutes}min)\n`;
          }

          // Special flags
          const flags = [];
          if (apt.isPomodoro) flags.push('🍅 Pomodoro');
          if (apt.isOvertime) flags.push('⏰ Encaixe');
          if (apt.allowOverlap) flags.push('🔄 Sobreposição permitida');
          if (!apt.isWithinWorkHours) flags.push('🌙 Fora do horário comercial');

          if (flags.length > 0) {
            exportText += `   🏷️ Tags: ${flags.join(', ')}\n`;
          }

          // Notes
          if (apt.notes) {
            exportText += `   📝 Observações: ${apt.notes}\n`;
          }
      ...
      exportText += `🎯 Total de itens na agenda: ${appointments.length}\n`;

      if (summary.totalDurationMinutes > 0) {
        const workloadPercentage = Math.round((summary.totalDurationMinutes / 480) * 100); // 480 min = 8h
        exportText += `⏰ Carga de trabalho: ${workloadPercentage}% (baseado em 8h/dia)\n`;
      }

      console.log(`✅ Daily schedule exported successfully for ${date}`);
      return exportText;
